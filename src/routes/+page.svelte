<script lang="ts">
	...
	let defaultSongs = [
		...
		{
			name: "Unknown Brain feat MIME - War Zone",
			url: "/brain.mp3",
			duration: "3:03",
			format: "flac",
			kbps: "320 kbps",
	...
	let backgroundStyle = `background-image: url(${backgroundImageUrls[currentImageUrlIndex]});`;
	...
	function loadTrack(trackIndex: number) {
		if (trackIndex >= 0 && trackIndex < playlist.length) {
			...
			currentTrackName = track.name.replace(/\.[^/.]+$/, "");
	...
	function updateLoopModeLabel() {
		if (loopMode === 0) {
			...
			loopModeLabel = "Playlist";
		} else if (loopMode === 2) {
			loopModeLabel = "Current Track";
		}
	}
	...
	const handleKeyDown = (event: { key: string; shiftKey: any }) => {
		if (event.key === " ") {
			...
			if (isBottomDrawerOpen)
				showNotification("Visualizer On (Hotkey: SHIFT)");
			...
			if (isRightDrawerOpen)
				showNotification("Open Playlist (Hotkey: P)");
			...
			toggleLoopMode();
			showNotification("Loop Mode: " + loopModeLabel + " (Hotkey: L)");
		}
	};
	...
	const changeBackgroundImage = () => {
		currentImageUrlIndex =
			(currentImageUrlIndex + 1) % backgroundImageUrls.length;
		backgroundStyle = `background-image: url(${backgroundImageUrls[currentImageUrlIndex]});`;
	};

	const backgroundImageChangeInterval = setInterval(
...
>
	...
	<div class="flex">
		...
		{#if isLeftDrawerOpen}
			...
			>
				<Settings {toggleModal} {toggleLoopMode} {loopModeLabel} />
				<!-- Left Drawer Content -->
			</div>
		{:else}
			<div class="hidden" />
		...
		<div class="h-96 w-96 rounded-lg">
			...
			{#if isBottomDrawerOpen}
				<div
					class="h-96 w-96 rounded-lg transform-gpu text-white absolute"
					transition:slide={{
						delay: 250,
						duration: 300,
						easing: quintInOut,
