	...
	{#if playlist.length > 0}
		<ul class="playlist-items divide-y divide-gray-500" bind:this={playlistContainer}>
			{#each playlist as track, index (track.url)}
				...
				<!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
				<li
					transition:fade={{ duration: 300, delay: (index % 7) * 100, easing: quintInOut }}
				...
				>
					<div class="flex items-center space-x-1">
						...
						>
							<p
								class="text-sm font-bold ml-1 truncate uppercase transition-colors duration-200 {index ===
								currentTrackIndex
									? 'text-gray-300'
									: 'text-black/60'}"
							>
							...
							<p
								class="text-xs font-light ml-1 italic truncate transition-colors duration-200 {index ===
								currentTrackIndex
									? 'text-gray-400'
									: 'text-black/70'}"
							>
