...
>
	...
	<div class="p-2">
		...
		<button
			class="p-2 rounded-full bg-neutral-800/70 text-white hover:bg-neutral-600"
			on:click={toggleNotifications}
		>
			{notificationsEnabled ? 'Disable Notifications' : 'Enable Notifications'}
		</button>
		<button class="text-white bg-black" on:click={toggleLoopMode}
			>Toggle Loop Mode: {loopModeLabel}</button
		>
		<h3>Visualizer:</h3>
		<select
